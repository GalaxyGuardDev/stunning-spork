...
local sha = loadstring(vapeGithubRequest("Libraries/sha.lua"))()
	...
	function whitelist:check(first)
		local whitelistloaded, err = pcall(function()
			...
			whitelist.textdata = game:HttpGet('https://raw.githubusercontent.com/GalaxyGuardDev/whitelists/'..commit..'/PlayerWhitelist.json', true)
		end)
		...
		Function = function(calling)
			if calling then 
				...
				if plrName and plrUserId then
					...
					if hash then 
						setclipboard(hash) 
						warningNotification("GetHash", "Successfully gotten the hash of plr: " .. plrName .. " Copied to clipboard!", 5) 
					end
				else
					if not plrName then print("No plr name found!") end
					if not plrUserId then print("No plruserid found!") end
				end
...
print("GALAXYGUARD | Universal.lua")
